...
papaja::r_refs("BA_R.bib")

old <- theme_set(theme_classic(base_size = 16))
...
beers %>%
  ...
  select("overall" = rAvg,
         ...
         "name" = short_name,
         "style" = beer_style) %>%
  gather(key = "Area", value = "Score", -c(name, style)) %>%
  mutate(Area = fct_rev(fct_relevel(Area, "aroma", "appearance", "palate", "taste", "overall"))) %>%
  ggplot(aes(x = Score, y = Area, label = name)) +
  geom_point() +
  facet_wrap(~name, nrow = 2) +
  scale_x_continuous(breaks = c(4, 4.5, 5), limits = c(4,5)) +
...
beers %>%
  ...
  top_n(10, score) %>%
  arrange(desc(ss)) %>%
  select("S.S." = ss, ABV, "name" = short_name) %>%
  ggplot(aes(x = ABV, y = `S.S.`, label = name)) +
  geom_label(nudge_y = 0.005) +
  geom_point() +
  coord_cartesian(xlim = c(0, 15), ylim = c(3.38, 3.62), expand = FALSE)
```
...
beers %>%
  ...
  geom_point() +
  coord_cartesian(xlim = c(0, 15.05), expand = FALSE) +
